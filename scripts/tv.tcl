set datadoscript "28-Junho-2020"
...
#	"d/a/b|mm hh|link"
    ...
    if {![file exists $tvfichcfg]} {
		...
		close $f
		putlog "TV: $tvfichcfg vazio criado"
    }
...
#        set urlz [lreplace $urlz $urlselec-1 $urlselec-1]
    ...
    close $f	
	putlog "TV: TVLISTAS usada por $nick no canal $chan: >$text<"
}
...
proc tvengine {} {
	...
	putlog "TV: A iniciar a tarefa..."
	set tarefainiciada [clock milliseconds]
	if {![file exists $tvfichcfg]} {
		set f [open $tvfichcfg w+]
		puts $f "actualizar=12:00"
		close $f
		putlog "TV: Ficheiro vazio criado."
	...
	foreach url $turls {
		...
		putlog "TV: OK. O $nomedoficheiro tem [string length $xmlc] bytes."
	
		set xmlc [split $xmlc "\n"]

	    putlog "TV: A remover lixo do xml..."
	    set xmlc [string map {
	        " lang=\"pt\"" "" " lang=\"en\"" "" "&amp;" "&" "&quot;" "\"" "&apos;" "'"
	        ".pt" "" ".PT" "" ".nws" "" " (\?)" "" "(n)" "" "  " "" "    " ""} $xmlc]
	    putlog "TV: [string length $xmlc] bytes após a limpeza."
		...
		putlog "TV: O ficheiro $nomedoficheiro contém $numcanais canais."
		lappend ltotalcanais $contador=$numcanais
...
#		putlog "TV: O ficheiro contém $numerocanais canais."

		#	putlog "TV: A criar array..."
		#	for {set linha 1} {$linha<=$numerocanais} {incr linha} {
		#		 for {set coluna 0} {$coluna<500} {incr coluna} {
		#			  set programacao($linha,$coluna) ""
		#		 }
		#	 }
		#	 putlog "TV: Array criado. [array size programacao] elementos."



		putlog "TV: A preencher o array (isto pode demorar algum tempo)..."
...
#           }
			...
			if {$linha=="</programme>"} {
				...
				lappend programacao($canal) $programa
			}
		}
	}
	...
	set estadofinal "Tarefa terminada em $tempodecorrido segundos. Canais obtidos por ID $ltotalcanais, Total canais obtidos: $numerototalcanais. $lcontador"
	putlog "TV: $estadofinal"
	return $estadofinal
}
...
proc tvsched {quando nick chan text} {
	...
	if {$text==""} {
		if {![file exists $fichnicks]} {
			...
			close $f
			putlog "TV: Ficheiro de nicks vazio criado."
		}
...
#putquick "privmsg $chan :>$itemtext<"
		...
		if {$conteudo==""} {
			putquick "privmsg $chan :Impossível mostrar o conteúdo. O guia de tv de origem não continha dados para este canal e talvez mais canais."
			return
		}
