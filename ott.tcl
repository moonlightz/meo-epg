...
foreach idcanal [lsort -increasing [array names canais]] {
	...
	for {set a 0} {$a<100} {incr a} {
		...
		foreach dprograma $cvalue {
			...
			puts -nonewline " [lindex "/ - \\\\ |" $isymb] [string replace [string repeat " " 40] 0 [string length $nomecanal]-1 $nomecanal]\r"
			flush stdout
			incr numprogs
			incr isymb
		...
		if {[catch {set link [dict get $cdict "odata.nextLink"]} erro]} {
			#Não há aquela chave, sair do loop
			puts " v [string replace [string repeat " " 40] 0 [string length $nomecanal]-1 $nomecanal] Feito! $numprogs programas."
			break
		}
	}
